...
def _sheet_graph(ws, S, ctx):
    ...
    STEP = 23                       # 블록 간 행 간격(제목-차트 사이 여백 포함)
    SIDE = 10                       # 차트 정사각 한 변(cm)
    CH_COL = "F"                    # 차트 시작 열

    from openpyxl.styles import PatternFill
    tan = PatternFill("solid", fgColor="DED9C4")   # 원본과 동일한 베이지 제목 막대

    def band(row, text):
        # 제목 막대 폭을 차트 폭(약 10cm ≈ F~K)에 맞춤
    ...
    def place(chart, top_row, span_rows=19):
        """제목 막대(F:K)와 동일한 열 범위로 차트를 고정(two-cell anchor)."""
        a = TwoCellAnchor(editAs="oneCell")
        a._from = AnchorMarker(col=5, colOff=0, row=top_row - 1, rowOff=0)       # F 좌상단
        a.to = AnchorMarker(col=11, colOff=0, row=top_row - 1 + span_rows, rowOff=0)  # K 우측 경계
        chart.anchor = a
        ws.add_chart(chart)

    # 블록 시작행: 2, 25, 48 (제목 → 그 아래 표/차트)
    b0, b1, b2 = 2, 2 + STEP, 2 + 2 * STEP
    ...
    radar = RadarChart(); radar.type = "standard"   # 선형 방사형(이미지처럼)
    ...
    rs = radar.series[0]
    rs.graphicalProperties = GraphicalProperties()
    rs.graphicalProperties.line = LineProperties(solidFill="4472C4", w=28575)
    # 축 라벨 강제 표시(openpyxl 기본은 숨김) → 항목명·% 눈금이 나오고 자동 여백 생김
    ...
    axis_gray(radar.y_axis)        # % 눈금 흐리게
    radar.legend = None
    place(radar, b0 + 2)   # 제목과 한 줄 띄우고, 제목과 동일 폭(F:K)
    ...
    bar.x_axis.delete = False      # x축 항목명(계정관리 등) 표시
    bar.y_axis.delete = False      # y축 눈금 표시
    bar.legend = None        # 단일 계열 + x축 라벨로 충분(이름 범례 제거)
    place(bar, b2 + 2)
