...
@lru_cache(maxsize=8)
def _page_texts(path_str: str) -> tuple[str, ...]:
    """PDF 각 페이지의 추출 텍스트를 한 번만 읽어 캐시(코드/섹션 추출 공용)."""
...
def _page_codes(path_str: str) -> tuple[tuple[int, frozenset[str]], ...]:
    ...
    return tuple((i, frozenset(_CODE_RE.findall(text)))
                 for i, text in enumerate(_page_texts(path_str)))
...
def section_text(path_str: str, code: str) -> str | None:
    ...
    if rng is None:
        return None
    start, end = rng
    texts = _page_texts(path_str)
    parts = texts[start:min(end + 1, len(texts))]
    text = "\n".join(parts).strip()
    return text or None


# '조치 방법' 헤딩(추출 시 글자 사이 공백이 섞일 수 있어 \s* 허용)
_REMED_RE = re.compile(r"조\s*치\s*방\s*법")
# '조치 방법' 다음에 오는 절(節) 헤딩 — 여기서부터는 pre-fill 에 넣지 않는다.
_REMED_END_RE = re.compile(r"조\s*치\s*시\s*영\s*향|점\s*검\s*및\s*조\s*치\s*사\s*례|조\s*치\s*사\s*례")


@lru_cache(maxsize=128)
def remediation_section(path_str: str, code: str) -> str | None:
    """항목 섹션에서 '조치 방법' 절의 본문만 잘라 반환('조치 사례'는 제외).

    AI 판정 전 보고서 '조치방법' 열을 미리 채우는 '로직 기반(가이드 매핑)' 용도.
    헤딩을 못 찾으면 None(=가이드 매핑 없음).
    """
    sec = section_text(path_str, code)
    if not sec:
        return None
    m = _REMED_RE.search(sec)
    if not m:
        return None
    rest = sec[m.end():]                 # '조치 방법' 헤딩 이후
    end = _REMED_END_RE.search(rest)     # 다음 절 헤딩 직전까지만
